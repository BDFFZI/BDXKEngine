...
float4 PixelPass(Pixel pixel) : SV_TARGET
{
    ...
    if(directionAbs.x > directionAbs.y && directionAbs.x > directionAbs.z)
    {
        ...
        if(direction.x > 0.5f)
        {
            return float4(Texture2D0.Sample(SamplerState0, float2(1 - direction.z,1 - direction.y)).xyz,1);
        }
        else
        {
            return float4(Texture2D1.Sample(SamplerState1, float2(direction.z,1 - direction.y)).xyz,1);
        }
    }
    else if(directionAbs.y > directionAbs.x && directionAbs.y > directionAbs.z)
    {
        ...
        {
            return float4(Texture2D3.Sample(SamplerState3, float2(direction.x,1-direction.z)).xyz,1);
        }
    }
    else if(directionAbs.z > directionAbs.x && directionAbs.z > directionAbs.y)
    {
        direction /= directionAbs.z;
        direction += float3(1,1,1);
        direction /= 2;
        if(direction.z > 0.5f)
        {
            return float4(Texture2D4.Sample(SamplerState4, float2(direction.x,1-direction.y)).xyz,1);
        }
        else
        {
            return float4(Texture2D5.Sample(SamplerState5, float2(1-direction.x,1-direction.y)).xyz,1);
        }
    }

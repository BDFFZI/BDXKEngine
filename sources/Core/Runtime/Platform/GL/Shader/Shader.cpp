...
namespace BDXKEngine
{
    ...
    void CompileShaderFromFile(const std::filesystem::path& path, const char* entrypoint, const char* target, ID3DBlob** blob)
    {
        if (exists(path) == false)
            throw std::exception("文件不存在");

        CComPtr<ID3DBlob> compileError;
        const HRESULT result = D3DCompileFromFile(path.wstring().c_str(),
                                                  nullptr,
    ...
    ObjectPtr<Shader> Shader::Create(const std::string& hlsl, const bool instantiate)
    {
    ...
    ObjectPtr<Shader> Shader::CreateFromFile(const std::filesystem::path& path, const bool instantiate)
    {
        ObjectPtr shader = new Shader{};

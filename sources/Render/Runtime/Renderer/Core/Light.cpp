...
namespace BDXKEngine
{
        ...
        if (lightType == LightType::Point)
        {
            ...
            for (int index = 0; index < 6; index++)
            {
                ...
                for (const ObjectPtr<Renderer>& renderer : rendererQueue)
                {
                    ...
                    if (renderer->GetMaterial(true)->UploadRP(PassType::ShadowCaster))
    ...
    void Light::Awake()
    {
        ...
        shadowMap = Texture2D::Create(4096, 4096, TextureFormat::R32G32B32A32_FLOAT);
        shadowMapCube = TextureCube::Create(512, 512, TextureFormat::R32G32B32A32_FLOAT);
    }
    void Light::Destroy()
    {
        DestroyObject(shadowMap);
        DestroyObject(shadowMapCube);

...
namespace BDXKEngine
{
    ...
    {
        const D2D1_ELLIPSE ellipse = D2D1::Ellipse({center.x, center.y}, radius, radius);
        if (isFill)
    ...
    void GL2D::DrawTextf(Rect rect, std::string text, int fontSize)
    {
        IDWriteTextFormat* textFormat;
        const LRESULT result = writeFactory->CreateTextFormat(L"simkai", nullptr,
                                                              DWRITE_FONT_WEIGHT_NORMAL, DWRITE_FONT_STYLE_NORMAL,
                                                              DWRITE_FONT_STRETCH_NORMAL,
                                                              static_cast<float>(fontSize), L"zh-cn", &textFormat);
        assert(SUCCEEDED(result)); //字体格式创建失败

        const D2D1_RECT_F d2d1Rect{rect.x, rect.y, rect.GetXMax(), rect.GetYMax()};
        renderTarget->DrawText(String::to_wstring(text).c_str(), static_cast<UINT32>(text.size()), textFormat, d2d1Rect, brush);

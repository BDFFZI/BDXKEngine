...
namespace BDXKEngine
{
    ...
    public:
        ...
        {
            for (const std::unordered_map allStartHandlersTemp(allStartHandlers); auto item : allStartHandlersTemp)
                if (item.second == false)allStartHandlers.erase(item.first);

            for (const std::unordered_map allUpdateHandlersTemp(allUpdateHandlers); auto item : allUpdateHandlersTemp)
                if (item.second == false)allUpdateHandlers.erase(item.first);

            for (const std::unordered_map allLateUpdateHandlersTemp(allLateUpdateHandlers); auto item :
                 allLateUpdateHandlersTemp)
                if (item.second == false)allLateUpdateHandlers.erase(item.first);

...
namespace BDXKEngine {
	Window* GUI::window;
	std::stringstream GUI::charStream{};
	...
	std::string GUI::TextArea(Rect rect, std::string text, int fontSize)
	{
		Color background;
	...
	float GUI::Slider(Rect rect, float value, float leftValue, float rightValue)
	{
		...
		GL2D::DrawTextf(rect, std::to_string(value), 20);
	...
	{
		GUI::window = window;
		window->AddCharacterEvent([](char character) {
			if (Event::HasFocus())

...
namespace BDXKEngine {
	StaticMesh MeshImporter::ImportGltf(char* fileName)
	{
		...
		rapidjson::Value& positionsAccessor = document["accessors"][document["meshes"][0]["primitives"][0]["attributes"]["POSITION"].GetUint()];
		...
		std::memcpy(stateMesh.positions.data(), buffer.get() + positionsBufferView["byteOffset"].GetUint(), positionsBufferView["byteLength"].GetUint());
		std::memcpy(stateMesh.normals.data(), buffer.get() + normalsBufferView["byteOffset"].GetUint(), normalsBufferView["byteLength"].GetUint());
		std::memcpy(stateMesh.uvs.data(), buffer.get() + uvsBufferView["byteOffset"].GetUint(), uvsBufferView["byteLength"].GetUint());
		std::memcpy(stateMesh.triangles.data(), buffer.get() + trianglesBufferView["byteOffset"].GetUint(), trianglesBufferView["byteLength"].GetUint());

		return stateMesh;
	}

	StaticMesh MeshImporter::Import(char* fileName)
	{
		return ImportGltf(fileName);
	}
